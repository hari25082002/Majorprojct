...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        # Display the recognized person's name or 'Unknown' if confidence is above the threshold
        if confidence < confidence_threshold:
            cv.putText(frame, str(people[label]), (x, y-10), cv.FONT_HERSHEY_COMPLEX, 1.0, (242,242, 12), thickness=2)
        else:
            cv.putText(frame, 'Unknown', (x, y-10), cv.FONT_HERSHEY_COMPLEX, 1.0, (0, 0, 255), thickness=2)
